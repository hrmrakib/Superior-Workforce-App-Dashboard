    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="inline-flex items-center gap-3 bg-white border border-slate-200 px-5 py-3 rounded-xl shadow-sm">
          <span className="text-sm font-medium text-slate-500">Total Notifications</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {notifications.map((notif) => {
          ...
          return (
            <div key={notif.id} className={`bg-white rounded-xl border p-5 relative group transition-shadow hover:shadow-md ${styles.border}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3 flex-wrap">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${styles.bg}`}>
                    {getGroupIcon(notif.group)}
                  </div>
                  
                  <div className="flex items-center gap-2 text-xs font-semibold">
                    {notif.group === "All Users" ? (
                      <span className={`px-2 py-0.5 rounded-md ${styles.tagBg} ${styles.tagText}`}>
                        All Users
                      </span>
                    ) : (
                      <>
                        ...
                        <span className="text-slate-400">•</span>
                        <span className="text-slate-500">{notif.roleOrCompany}</span>
                      </>
                    )}
                  </div>
                </div>
              ...
              <h3 className="text-base font-bold text-slate-800 mb-2">{notif.title}</h3>
