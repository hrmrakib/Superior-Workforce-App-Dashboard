          ...
          <div className="relative" ref={dropdownRef}>
                  ...
                  {selectedUsers.length === 0 ? (
                    ...
                    <>
                      ...
                      {selectedUsers.length > 1 && (
                        <span className="text-slate-700">+{selectedUsers.length - 1}</span>
                      )}
                    </>
                  )}
                </div>
              )}
              {dropdownOpen ? <ChevronUp className="w-4 h-4 text-slate-400 shrink-0 ml-2" /> : <ChevronDown className="w-4 h-4 text-slate-400 shrink-0 ml-2" />}
            </div>
            ...
            {dropdownOpen && (
              <div className="absolute top-full left-0 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-lg z-10 p-2 overflow-hidden">
                <button 
                  type="button"
                  onClick={() => { setTargetMode("All Users"); setDropdownOpen(false); }}
                ...
                <div className="mt-1">
                  <button 
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm font-medium rounded-lg transition-colors ${
                      targetMode === "Specific Users" ? "text-blue-600" : "text-slate-600 hover:bg-slate-50"
                    }`}
                  >
                    Specific Users
                  </button>
                      ...
                      <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
                        ...
                        {selectedUsers.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-3">
                            {selectedUsers.map(u => (
                              <div key={u} className="flex items-center gap-1.5 px-3 py-1 bg-teal-700 text-white text-xs font-medium rounded-full">
                                {u}
                                <button onClick={() => handleRemoveUser(u)} className="hover:text-teal-200">
                                  <X size={12} />
                                </button>
                              </div>
                            ))}
                        ...
                        <div className="flex flex-wrap gap-2 mt-4">
                          {mockUsers.filter(u => !selectedUsers.includes(u)).map(u => (
                            <button
                              key={u}
                              type="button"
