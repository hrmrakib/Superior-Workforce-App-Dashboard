...
import { Search, ChevronDown, MapPin, Users, DollarSign, Clock, Timer, Building2, UserCircle, Briefcase, BarChart3 } from "lucide-react";
...
export default function ManageJobsPage() {
  ...
  const [activeTab, setActiveTab] = useState<JobStatus | "All Jobs">("All Jobs");
  const [currentPage, setCurrentPage] = useState(1);

  const filteredJobs = mockJobs.filter((job) => {
    const matchesSearch = job.hospital.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 overflow-x-auto bg-slate-50 p-1 rounded-xl border border-slate-200">
          {(["All Jobs", "Ongoing", "Upcoming", "Completed"] as const).map((tab) => (
            <button
              key={tab}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {paginatedJobs.map((job) => (
          <div key={job.id} className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-sm transition-shadow">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-slate-800 text-base">{job.hospital}</h3>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-sm font-semibold text-blue-600">{job.role}</span>
